...
public class DaoFile implements IDao {
	...
	public DaoFile() {
		this.filename = "data2\\membres.xml";
	}
...
findAll();
	...
	public void update(Person p) {
		...
		while(it.hasNext()) {
			...
			if(currentPerson.getId().equals(p.getId())) {  // a revoir le getId dans Person !!!!!
				it.remove();
				trouve = true;
			}
		}
	...
	public void update(UUID id) {
		...
		while(it.hasNext()) {
			...
			if(currentPerson.getId().equals(id)) {  // a revoir le getId dans Person !!!!!
				it.remove();
				trouve = true;
			}
		}
